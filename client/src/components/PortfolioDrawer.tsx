...
import { MARKET_PROFILE_ORDER, MARKET_PROFILES, taxReviewDescription, type MarketProfile, type MarketProfileId } from "@shared/marketProfiles";
...
type PortfolioDrawerProps = {
  ...
  setMarketId: (id: MarketProfileId) => void;
  ...
  showHalos: boolean;
  setShowHalos: (value: boolean) => void;
  showCostumes: boolean;
  setShowCostumes: (value: boolean) => void;
...
export default function PortfolioDrawer({
  ...
  setTaxFilter, showEtfs, setShowEtfs, marketId, setMarketId, marketProfile, darkMode, litterboxRef,
  etfLotCount, frozen, setFrozen, reducedMotion, repulsion, setRepulsion,
  gravityOn, setGravityOn, showPnlBadges, setShowPnlBadges, showHalos,
  setShowHalos, showCostumes, setShowCostumes, onImportFile,
    ...
    <Sheet modal={false} open={open} onOpenChange={setOpen}>
      ...
      <SheetContent id="portfolio-controls-pane" side="right" aria-describedby={undefined} onPointerDownOutside={(event) => {
        ...
        <div className="space-y-7 overflow-y-auto px-6 pb-10 pt-6">
          ...
          <section><p className="control-label">Market</p><div className="mt-3 grid grid-cols-2 gap-2">{MARKET_PROFILE_ORDER.map((id) => { const profile = MARKET_PROFILES[id]; return <button key={id} type="button" aria-pressed={marketId === id} onClick={() => setMarketId(id)} className={marketId === id ? "min-h-11 rounded-xl bg-stone-900 px-3 py-2 text-left font-mono text-[10px] text-white shadow-sm" : "min-h-11 rounded-xl border border-stone-200 bg-white px-3 py-2 text-left font-mono text-[10px] text-stone-600 transition hover:border-stone-400 hover:text-stone-900"}><span className="block font-semibold">{profile.label}</span><span className={marketId === id ? "mt-1 block text-[9px] text-stone-300" : "mt-1 block text-[9px] text-stone-400"}>{taxReviewDescription(profile)}</span></button>; })}</div><p className="mt-2 font-mono text-[9px] leading-4 text-stone-400">Re-dresses currency, grouping, dates, and the aged-loss flag. Numbers never change.</p></section>
          ...
          <section><p className="control-label">Field ornaments</p><div className="mt-1 flex items-center justify-between gap-4 rounded-xl border border-stone-200 bg-white px-4 py-3"><div><label htmlFor="show-halos" className="block font-mono text-[10px] text-stone-700">Show emphasis halos</label><p className="mt-1 font-mono text-[9px] leading-4 text-stone-400">Dashed ring on loss cats, solid on gains. Off by default — they read as visual noise in a dense field.</p></div><Switch id="show-halos" checked={showHalos} onCheckedChange={setShowHalos} aria-label="Show emphasis halos around kitties" /></div><div className="mt-2 flex items-center justify-between gap-4 rounded-xl border border-stone-200 bg-white px-4 py-3"><div><label htmlFor="show-costumes" className="block font-mono text-[10px] text-stone-700">Costumes</label><p className="mt-1 font-mono text-[9px] leading-4 text-stone-400">One story per cat; an ETF basket may join it. Pigment and size stay unchanged.</p></div><Switch id="show-costumes" checked={showCostumes} onCheckedChange={setShowCostumes} aria-label="Show costume layers on kitties" /></div></section>
