...
import { ChevronDown, Eye, FileUp, Pause, Play } from "lucide-react";
...
type PortfolioDrawerProps = {
  ...
  portfolioTotals: { invested: number; current: number; pnl: number; pnlPercent: number };
};
...
export default function PortfolioDrawer({
  ...
  uploadNotice, hasDates, onRestore, visibleKittyCount, loadedLotCount, portfolioTotals,
}: PortfolioDrawerProps) {
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetTrigger asChild>
        <button type="button" aria-label="Open portfolio controls" className="ekitty-menu-trigger fixed right-5 top-0 z-50 transition duration-200 hover:-translate-y-0.5 active:scale-95"><img src={litterBoxIcon} alt="" className={darkMode ? "h-14 w-14 invert" : "h-14 w-14"} /></button>
      </SheetTrigger>
      <SheetContent side="right" className="w-[min(390px,94vw)] gap-0 overflow-y-auto border-l border-stone-200 bg-[#fcfcfb] p-0 shadow-[-20px_0_60px_-34px_rgba(41,37,36,.48)] sm:max-w-none">
        <SheetHeader className="border-b border-stone-200 px-6 pb-5 pt-6">
          ...
          {loadedLotCount > 0 && <div className="mt-5"><p className="font-[Unbounded] text-[28px] font-700 leading-none tracking-tight text-stone-900">{formatCurrency(portfolioTotals.current)}</p><div className="mt-2 flex items-baseline gap-2"><span className={portfolioTotals.pnl >= 0 ? darkMode ? "font-mono text-[13px] font-medium text-[#4ade80]" : "font-mono text-[13px] font-medium text-emerald-700" : darkMode ? "font-mono text-[13px] font-medium text-[#ff6b6b]" : "font-mono text-[13px] font-medium text-[#ff3b3b]"}>{formatCurrency(Math.abs(portfolioTotals.pnl))}</span><span className={portfolioTotals.pnl >= 0 ? darkMode ? "font-mono text-[10px] text-[#4ade80]" : "font-mono text-[10px] text-emerald-700" : darkMode ? "font-mono text-[10px] text-[#ff6b6b]" : "font-mono text-[10px] text-[#ff3b3b]"}>{Math.abs(portfolioTotals.pnlPercent).toFixed(2)}%</span></div></div>}
        </SheetHeader>
