...
type PortfolioDrawerProps = {
  ...
  kittyScale: KittyScaleMetric;
  setKittyScale: (metric: KittyScaleMetric) => void;
  taxFilter: TaxFilter;
...
export default function PortfolioDrawer({
  open, onOpenChange, viewMode, setViewMode, colorMetric, setColorMetric, kittyScale, setKittyScale, taxFilter,
    ...
    <Sheet modal={false} open={open} onOpenChange={setOpen}>
      ...
      <SheetContent id="portfolio-controls-pane" side="right" aria-describedby={undefined} onPointerDownOutside={(event) => {
        ...
        <div className="space-y-7 overflow-y-auto px-6 pb-10 pt-6">
          ...
          <section><p className="control-label">Visual mapping</p><label className="field-label mt-3" htmlFor="kitty-scale">Kitty scale</label><div className="select-wrap"><select id="kitty-scale" value={kittyScale} onChange={(event) => setKittyScale(event.target.value as KittyScaleMetric)}><option value="invested">Invested position value</option><option value="current">Current market value</option><option value="quantity">Quantity held</option><option value="pnlMagnitude">Absolute P&amp;L magnitude</option></select><ChevronDown size={14} /></div><label className="field-label mt-4" htmlFor="color-map">Body pigment</label><div className="select-wrap"><select id="color-map" value={colorMetric} onChange={(event) => setColorMetric(event.target.value as ColorMetric)}><option value="absolute">Absolute P&amp;L — percentile</option><option value="percent">P&amp;L percentage — percentile</option></select><ChevronDown size={14} /></div><p className="mt-2 font-mono text-[9px] leading-4 text-stone-400">Scale and pigment are independent. Tiny P&amp;L remains neutral.</p></section>
