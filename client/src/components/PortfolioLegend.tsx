...
export default function PortfolioLegend({ darkMode, onClose, onOpen, visualLens, panelRef }: { darkMode: boolean; onClose: () => void; onOpen?: () => void; visualLens: VisualLens; panelRef?: RefObject<HTMLElement | null> }) {
  ...
  return (
    <aside ref={panelRef} role="dialog" aria-modal="false" aria-labelledby="portfolio-legend-title" className={`fixed top-[15.5rem] z-50 w-[min(300px,calc(100vw-24px))] rounded-xl border p-4 shadow-[0_18px_46px_-28px_rgba(41,37,36,.55)] ${panel}`} style={{ right: "calc(3rem + 44px + 12px)" }}>
