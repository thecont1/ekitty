...
const PORTFOLIO_CSV_URL = "/manus-storage/Portfolio Holdings Transactions_c29ac34e.csv";
    ...
    <button type="button" aria-label={`${point.company}: ${formatCurrency(point.pnl)} unrealized profit and loss`} className="group absolute z-10 block origin-center border-0 bg-transparent p-0 outline-none focus-visible:z-30 focus-visible:outline-none" style={{ width: size, height: size, transform: "translate(-50%, -50%)" }} onMouseEnter={onHover} onFocus={onHover} onMouseLeave={onLeave} onBlur={onLeave} onClick={onClick}>
      <span className="relative block h-full w-full transition-transform duration-200 ease-out group-hover:scale-[1.055] group-focus-visible:scale-[1.055]" style={{ transform: `rotate(${lean}deg) skewX(${skew}deg) scale(${widthScale}, ${heightScale})` }}>
        <span className="relative block h-full w-full" style={bobStyle}>
          ...
          {showAgeBadge && yearsHeld >= 1 && <span className="age-badge absolute right-[13%] top-[7%] grid h-[25%] min-h-4 w-[25%] min-w-4 place-items-center rounded-full border border-[#D8AE37] bg-white/95 text-stone-800 shadow-[0_2px_8px_rgba(41,37,36,.10)]">{yearsHeld}</span>}
        </span>
      </span>
    </button>
  );
...
export default function Home() {
  ...
  return (
    <main className="relative h-[100dvh] w-screen overflow-hidden bg-white text-stone-900">
      {viewMode === "transactions" && <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 overflow-hidden">{timeline.months.map((month, index) => <div key={month} className="absolute bottom-0 top-0 border-l border-[#edf7ff]" style={{ left: `${(index / timeline.months.length) * 100}%` }}><span className="absolute left-1 top-3 hidden font-mono text-[8px] tracking-[.12em] text-[#c3dff5] md:block">{labelMonth(month)}</span></div>)}</div>}
