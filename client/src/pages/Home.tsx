...
export default function Home() {
  ...
  const handleFieldPointerMove = useCallback((event: React.PointerEvent<HTMLElement>) => {
    setMousePosition({ x: event.clientX, y: event.clientY, visible: true });
  ...
  return (
    <main className={darkMode ? "dark relative h-[100dvh] w-screen overflow-hidden bg-[#101617] text-stone-100" : "relative h-[100dvh] w-screen overflow-hidden bg-white text-stone-900"}>
      ...
      <section ref={kittyWorld} aria-label="Portfolio kitty field" className="absolute left-0 top-0 z-10 touch-none" style={{ width: virtualCanvasWidth, height: virtualCanvasHeight, cursor: MOUSE_CURSOR }} onWheel={scrollTimeline} onPointerDown={(event) => { setMousePosition({ x: event.clientX, y: event.clientY, visible: true }); beginTimelineDrag(event); }} onPointerMove={handleFieldPointerMove} onPointerEnter={(event) => setMousePosition({ x: event.clientX, y: event.clientY, visible: true })} onPointerLeave={() => setMousePosition((current) => ({ ...current, visible: false }))} onPointerUp={endTimelineDrag} onPointerCancel={endTimelineDrag}>
      ...
      {mousePosition.visible && <div aria-hidden="true" className="pointer-events-none fixed z-[60] -translate-x-1/2 -translate-y-1/2" style={{ left: mousePosition.x, top: mousePosition.y }}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={darkMode ? "#f4f1ea" : "#2d302f"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5.5 3a3.5 3.5 0 0 1 3.25 4.8a7.017 7.017 0 0 0 -2.424 2.1a3.5 3.5 0 1 1 -.826 -6.9z" /><path d="M18.5 3a3.5 3.5 0 1 1 -.826 6.902a7.013 7.013 0 0 0 -2.424 -2.103a3.5 3.5 0 0 1 3.25 -4.799z" /><path d="M12 14m-7 0a7 7 0 1 0 14 0a7 7 0 1 0 -14 0" /></svg></div>}
