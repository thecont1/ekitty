...
const PORTFOLIO_CSV_URL = "/manus-storage/Portfolio Holdings Transactions_bc927e8a.csv";
...
function CatGlyph({ point, size, stroke, treatment, bobDuration, focused, frozen, showAgeBadge, onHover, onLeave, onClick }: VisiblePoint & { focused: boolean; frozen: boolean; showAgeBadge: boolean; onHover: () => void; onLeave: () => void; onClick: () => void }) {
  ...
  return (
    <button type="button" aria-label={`${point.company}: ${formatCurrency(point.pnl)} unrealized profit and loss`} className="group absolute z-10 block origin-center border-0 bg-transparent p-0 outline-none focus-visible:z-30 focus-visible:outline-none" style={{ width: size, height: size, transform: "translate(-50%, -50%)" }} onMouseEnter={onHover} onFocus={onHover} onMouseLeave={onLeave} onBlur={onLeave} onClick={onClick}>
      <span className="relative block h-full w-full transition-transform duration-200 ease-out group-hover:scale-[1.055] group-focus-visible:scale-[1.055]" style={{ transform: `rotate(${lean}deg) skewX(${skew}deg) scaleX(${widthScale})` }}>
        <span className="relative block h-full w-full" style={bobStyle}>
          ...
          <PortfolioKittySvg stroke={treatment.ink} fill={treatment.fill} fillOpacity={treatment.fillOpacity} strokeWidth={stroke} className="block h-full w-full overflow-visible drop-shadow-[0_5px_4px_rgba(41,37,36,.12)]" />
          {point.taxSensitive && <span className="absolute left-[42%] top-[53%] h-[16%] w-[16%] rounded-full border border-[#D8AE37]" />}
          {showAgeBadge && yearsHeld >= 1 && <span className="age-badge absolute right-[13%] top-[7%] grid h-[25%] min-h-4 w-[25%] min-w-4 place-items-center rounded-full border border-white bg-stone-900 text-white shadow-sm">{yearsHeld}</span>}
        </span>
      </span>
    </button>
  );
...
export default function Home() {
  ...
  const timeline = useMemo(() => createTimeline(asTransactionPoints(records)), [records]);
  ...
  const visiblePoints = useMemo<VisiblePoint[]>(() => {
    ...
    const minSize = compactField ? (viewMode === "transactions" ? 19 : 27) : (viewMode === "transactions" ? 28 : 40);
    const maxSize = compactField ? (viewMode === "transactions" ? 65 : 96) : (viewMode === "transactions" ? 108 : 196);
    ...
    const tick = (time: number) => {
      ...
      if (!frozen) {
        ...
        nodeList.forEach(({ node, size }) => {
          const margin = Math.max(25, size * 0.31);
  ...
  return (
    <main className="relative h-[100dvh] w-screen overflow-hidden bg-white text-stone-900">
      {viewMode === "transactions" && <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 overflow-hidden">{timeline.months.map((month, index) => <div key={month} className="absolute bottom-0 top-0 border-l border-[#d9ecff]" style={{ left: `${(index / timeline.months.length) * 100}%` }}><span className="absolute left-1 top-3 hidden font-mono text-[8px] tracking-[.12em] text-[#a6c8e8] md:block">{labelMonth(month)}</span></div>)}</div>}
      <section aria-label="Portfolio kitty field" className="absolute inset-0 z-10">
        {visiblePoints.map((entry) => {
          ...
          return <div key={entry.point.id} className={muted ? "opacity-25 grayscale-[.32] transition-opacity duration-300" : "transition-opacity duration-300"} style={{ position: "absolute", left: node.x, top: node.y }}><CatGlyph {...entry} focused={selectedId === entry.point.id} frozen={frozen} showAgeBadge={viewMode === "transactions"} onHover={() => setHoveredId(entry.point.id)} onLeave={() => setHoveredId((current) => current === entry.point.id ? null : current)} onClick={() => { setSelectedId(entry.point.id); setHoveredId(null); }} /></div>;
        })}
      </section>
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}><SheetTrigger asChild><button type="button" aria-label="Open portfolio controls" className="fixed right-5 top-5 z-50 grid h-12 w-12 place-items-center rounded-full border border-stone-200 bg-white shadow-[0_10px_30px_-16px_rgba(41,37,36,.45)] transition duration-200 hover:-translate-y-0.5 hover:border-stone-300 hover:shadow-[0_13px_33px_-15px_rgba(41,37,36,.5)] active:scale-95"><Menu size={20} strokeWidth={1.8} /></button></SheetTrigger><SheetContent side="right" className="w-[min(390px,94vw)] gap-0 overflow-y-auto border-l border-stone-200 bg-[#fcfcfb] p-0 shadow-[-20px_0_60px_-34px_rgba(41,37,36,.48)] sm:max-w-none"><SheetHeader className="border-b border-stone-200 px-6 pb-5 pt-6"><div className="flex items-center gap-3"><PortfolioKittySvg stroke="#ff3b3b" fill="transparent" fillOpacity={0} strokeWidth={2} className="h-10 w-10" /><div><SheetTitle className="font-serif text-[25px] font-medium tracking-tight">ekitty</SheetTitle><p className="mt-0.5 font-mono text-[9px] uppercase tracking-[.18em] text-stone-400">shares &amp; ETFs, living field</p></div></div></SheetHeader><div className="space-y-7 px-6 py-6"><section><p className="control-label">View field</p><div className="mt-3 grid grid-cols-2 rounded-xl border border-stone-200 bg-white p-1">{([ ["holdings", "Grouped holdings"], ["transactions", "Transactions"] ] as const).map(([mode, label]) => <button key={mode} type="button" onClick={() => { setViewMode(mode); setSelectedId(null); }} className={viewMode === mode ? "rounded-lg bg-stone-900 px-2 py-2.5 font-mono text-[10px] text-white shadow-sm" : "rounded-lg px-2 py-2.5 font-mono text-[10px] text-stone-500 transition hover:text-stone-900"}>{label}</button>)}</div></section><section><p className="control-label">Visual mapping</p><p className="field-label mt-3">Kitty scale</p><div className="mt-1 rounded-xl border border-stone-200 bg-white px-3 py-3 font-mono text-[10px] text-stone-700">Current position value</div><label className="field-label mt-4" htmlFor="color-map">Body pigment</label><div className="select-wrap"><select id="color-map" value={colorMetric} onChange={(event) => setColorMetric(event.target.value as ColorMetric)}><option value="absolute">Absolute P&amp;L — percentile</option><option value="percent">P&amp;L percentage — percentile</option></select><ChevronDown size={14} /></div><p className="mt-2 font-mono text-[9px] leading-4 text-stone-400">&lt;p2 remains neutral; &gt;p98 reaches the strongest green or {LOSS_RED}.</p></section><section><p className="control-label">Tax-loss lens</p><div className="mt-3 grid grid-cols-3 gap-1 rounded-xl border border-stone-200 bg-white p-1">{([ ["all", "All"], ["highlight", "Mark"], ["isolate", "Only"] ] as const).map(([filter, label]) => <button key={filter} type="button" onClick={() => { setTaxFilter(filter); setSelectedId(null); }} className={taxFilter === filter ? "rounded-lg bg-[#D8AE37] px-1 py-2.5 font-mono text-[10px] text-stone-900 shadow-sm" : "rounded-lg px-1 py-2.5 font-mono text-[10px] text-stone-500 transition hover:text-stone-900"}>{label}</button>)}</div><p className="mt-2 font-mono text-[9px] leading-4 text-stone-400">Gold rings flag loss lots dated 330+ days ago.</p></section><section><div className="flex items-center justify-between"><p className="control-label">Field motion</p><button type="button" onClick={() => setFrozen((current) => !current)} className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[.11em] text-stone-600 transition hover:border-stone-400">{frozen ? <Play size={11} /> : <Pause size={11} />}{frozen ? "Resume" : "Freeze"}</button></div><div className="mt-4 flex items-center gap-3"><span className="font-mono text-[10px] text-stone-500">Soft</span><input aria-label="Repulsion strength" type="range" min="0.2" max="1" step="0.05" value={repulsion} onChange={(event) => setRepulsion(Number(event.target.value))} className="kitty-range flex-1" /><span className="font-mono text-[10px] text-stone-500">Spacious</span></div></section><section><p className="control-label">Portfolio CSV</p><label className="mt-3 flex cursor-pointer items-center justify-between rounded-xl border border-dashed border-stone-300 bg-white px-4 py-4 transition hover:border-stone-500 hover:bg-stone-50" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); importFile(event.dataTransfer.files[0]); }}><span><span className="block font-mono text-[10px] uppercase tracking-[.12em] text-stone-700">Import or drop a file</span><span className="mt-1 block font-mono text-[9px] text-stone-400">company · buy_qty · avg_price · current_price · buy_date</span></span><FileUp size={18} className="text-stone-500" /><input type="file" accept=".csv,text/csv" className="sr-only" onChange={(event) => importFile(event.target.files?.[0])} /></label>{uploadNotice && <p className={uploadNotice.kind === "error" ? "mt-2 font-mono text-[9px] leading-4 text-[#ff3b3b]" : "mt-2 font-mono text-[9px] leading-4 text-emerald-700"}>{uploadNotice.message}</p>}{!timeline.hasDates && <p className="mt-2 font-mono text-[9px] leading-4 text-stone-400">The included CSV has no buy_date column. Import dated lots to activate literal month placement and year badges.</p>}<button type="button" onClick={() => { setRecords(defaultPortfolio); setUploadNotice(null); setSelectedId(null); setHoveredId(null); }} className="mt-3 inline-flex items-center gap-2 font-mono text-[10px] text-stone-500 transition hover:text-stone-900"><RotateCcw size={12} /> Restore included field</button></section><section className="border-t border-stone-200 pt-5"><div className="flex items-center justify-between font-mono text-[10px]"><span className="text-stone-400">Showing</span><span className="text-stone-700">{visiblePoints.length} kitties · {records.length} lots</span></div><div className="mt-2 flex items-center gap-1.5 font-mono text-[9px] text-stone-400"><Eye size={11} /> In Transactions, each vertical blue strip is one month.</div></section></div></SheetContent></Sheet>
    </main>
  );
}
